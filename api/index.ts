...
app.get("/api/store/order/track/:query", (req, res) => {
  try {
    ...
    const matched = orders.find(o => {
      ...
      return oId === cleanQuery || oNum === cleanQuery || oTrx === cleanQuery || oToken === cleanQuery;
    });

    if (!matched) {
      return res.status(404).json({ success: false, error: `Order '${rawQuery}' not found in database.` });
    }
    ...
    res.status(500).json({ success: false, error: err.message || "Failed to query order status" });
  }
});
